...
- **Landing / Explore split (2026-06-13):** `/` (`app/page.tsx`) is now a promo landing page — pitch headline ("Open to go anywhere, on any free day."), a live-fares `DepartureBoard`, CTA → `/explore`. `/explore` (`app/explore/page.tsx`) is the moved browsing page: board + grid. Explore sort is **cheapest-only** (no Best score / Most trips); the old region chip row + text input are replaced by one `SearchCombobox` (city / country / region typeahead, `components/explore/SearchCombobox.tsx` → `SearchSelection`); the "Only my free dates" toggle sits in the search row's trailing slot. Nav "Explore" + city back-link point at `/explore`; the wordmark points at `/`.
- **Landing board mix (2026-07-23):** the six-row promo board is intentionally not the raw cheapest six. `lib/landing-board.ts` selects the cheapest fare overall, the cheapest fare from EIN (unless the global winner already leaves EIN), then the cheapest live tier-A tourist destinations / major hubs. It falls back to lower-tier cheap fares only when the live tier-A pool cannot fill the board.
...
- **Admin information architecture redesign (2026-07-23):** the admin area is now three read-only views: `/admin` Overview (signup/engagement/favourite analytics + live scraper summary), `/admin/people` People (setup-score filtering, responsive roster, person/group cross-linked sheets), and `/admin/pool` Pool (live board, compact health strip, searchable/filtered targets, collapsible run feed, compact two-step danger row). `/admin/users` redirects to `/admin/people`. Setup score is one point each for onboarding, availability, favourites, and social (friend or group). `/api/admin/pool` emits per-target `overdue`; `/api/admin/users` adds group activity/favourite/trip summary fields. There is still no group-trip persistence, so stored embedded trips are exposed only when present and group activity otherwise uses the latest trustworthy group/member/invite timestamp. The only mutation remains the existing flight wipe.
- **Calendar ±2-day default (2026-07-23):** the Calendar’s “± 2 days” availability expansion defaults on for new and untouched accounts. An explicit toggle is saved immediately as `preferences.calendar_near_miss`, so opting out persists across sessions and devices; missing legacy values resolve to `true`.
- **Country favourites + settings autosave (2026-07-23):** Settings/onboarding use a persistent multi-select search with country matches above all of their catalogued cities; choosing results keeps the query/menu open. Country selections still expand into `users.saved_cities` for existing fare queries, while `users.saved_countries` preserves the country as one expandable container when individual airports are unchecked. Its gold fill shows the selected-airport percentage, and the expanded block supports per-airport edits plus Select/Deselect all. A 0/N country remains as an editable draft only while its panel is open, then disappears when the user closes/leaves it; focusing search also closes the country panel. Preference changes autosave through a serialized client queue; the manual “Save preferences” button is removed.
- **Shared availability setup (2026-07-23):** Settings and onboarding now use the same `AcademicCard` + `YearPaint` layout. The TU/e student toggle sits beside Clear all and immediately shows/hides both the academic overlays and their inline exam/holiday legend. Settings departure airports moved into the upper-right of the Availability card above the calendar; the old standalone Preferences section is gone. `/api/preferences` accepts validated partial updates so airport, TU/e, and other autosaves cannot overwrite one another with stale full preference objects.
