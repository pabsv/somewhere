...
## Corpus Check
- 162 files · ~77,705 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1372 nodes · 2531 edges · 93 communities (83 shown, 10 thin omitted)
- Extraction: 98% EXTRACTED · 2% INFERRED · 0% AMBIGUOUS · INFERRED: 61 edges (avg confidence: 0.69)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `b10a70b5`
...
8. `RouteStats` - 24 edges
9. `FlightRepository` - 24 edges
10. `Trip` - 22 edges
...
Cohesion: 0.07
Nodes (55): GET(), citiesForOrigins(), GET(), DateStr, GET(), QuerySchema, Origin, ORIGINS (+47 more)

### Community 2 - "Community 2"
Cohesion: 0.05
Nodes (44): ExplorePage(), matchesSelection(), LandingPage(), BoardSkeleton(), DepartureBoardProps, DepartureRow, FlapText(), FlapTextProps (+36 more)
...
Cohesion: 0.11
Nodes (24): Schedule state repository.  One document per origin airport, upserted on each, Insert or update schedule state for an origin., Return state for all origins, sorted by origin code., Return state for a single origin., Remove all schedule state (e.g. after changing the origin list)., ScheduleRepository, Scheduler module for automated flight scraping., configure_scheduler() (+16 more)
...
Cohesion: 0.10
Nodes (29): get_current_user_id(), Shared dependencies for the API.  Multi-user mode: identity is established via, FastAPI dependency: reads user ID from the X-User-ID request header.     Return, DateWindow, get_preferences(), BaseModel, GET  /api/preferences — read the full UserPreferences object PUT  /api/preferen, save_preferences() (+21 more)
...
Cohesion: 0.10
Nodes (19): get_connection_string(), MongoDB configuration settings.  Uses environment variables with sensible defa, Build MongoDB connection string., close_connection(), MongoDB connection manager.  Singleton pattern to ensure a single database con, Close the database connection., Flight Scraper Database Layer  Provides MongoDB models, repositories, and serv, Flight model v2 — one document per itinerary date-pair.  flight_key no longer in (+11 more)
...
Cohesion: 0.16
Nodes (8): Create User from MongoDB document., User, Update an existing user.          Args:             user: User object with up, Find an existing user by email or create a new one (no password required)., Find a user by email., Find all active users., Find all users regardless of active status., Find all users who have a specific destination in their preferences.         No

### Community 16 - "Community 16"
Cohesion: 0.20
Nodes (5): User model with embedded airports and notification preferences., User's airport preferences., User's notification preferences., UserAirports, UserNotifications

### Community 17 - "Community 17"
Cohesion: 0.17
Nodes (18): AgendaMonth(), clampDayInMonth(), dayStr(), isWeekend(), MONTHS_LONG, pad(), spansMonth(), toDateStr() (+10 more)
...
Cohesion: 0.06
Nodes (32): ScrapeTarget model — one document per (origin, destination) route in the pool., ScrapeTargetModel, Most recent runs, newest first., ScrapeRunRepository, datetime, ScrapeTargetRepository — manages the route pool.  Core operations:   bulk_upsert, Atomically claim the next-due enabled target.          Marks it as 'running' by, Finalize a target after a scrape attempt.          Computes next_due_at from tie (+24 more)

### Community 20 - "Community 20"
Cohesion: 0.14
Nodes (12): CalendarPage(), CalendarFiltersProps, CalendarFilterState, EMPTY_FILTERS, TierFilter, TIERS, addMonths(), monthSpan() (+4 more)

### Community 21 - "Community 21"
Cohesion: 0.19
Nodes (17): BestPerMonth(), buildSlots(), MONTHS_SHORT, nightsBetween(), TIER_BAR, TripBar(), TIER_BADGE, TripPopover() (+9 more)
...
Cohesion: 0.24
Nodes (11): FriendsPage(), Mode, Status, getFriends(), getUsers(), removeFriend(), request(), respondToFriendRequest() (+3 more)

### Community 24 - "Community 24"
Cohesion: 0.27
Nodes (6): clampNights(), Mode, PreferencesCard(), getPreferences(), putPreferences(), Preferences

### Community 25 - "Community 25"
Cohesion: 0.06
Nodes (33): AdminTargetSummarySchema, CityBest, CityBestSchema, CitySummarySchema, DateStringSchema, DealTierSchema, DirectoryUserSchema, FlightDocSchema (+25 more)
...
Cohesion: 0.21
Nodes (8): LiveBoard(), RunFeed(), Status, STATUS_STYLE, timeAgo(), adminRuns(), AdminRunsResponse, ScrapeRunSummary
...
Cohesion: 0.15
Nodes (13): findGroupForMember(), GroupDoc, GroupMember, loadGroupDetail(), loadGroupsState(), memberOriginsUnion(), resolveInviteToken(), GroupDetailResponse (+5 more)
...
Cohesion: 0.23
Nodes (11): BestPerMonthProps, MonthSlot, AgendaMonthProps, WeekGroup, MonthSpec, DaySheetProps, MonthBlockProps, TripBarProps (+3 more)
...
Cohesion: 0.14
Nodes (7): Destination preference model - user's preferred travel destinations., Route statistics model - aggregated price data for routes., ScrapeRun model — one document per scraper execution of one route.  Pure observa, ScrapeRunModel, User's search preferences., UserSearchPreferences, Insert a 'running' record. Returns the inserted _id as str.
...
Cohesion: 0.43
Nodes (5): DaySheet(), Sheet(), SheetProps, getTrips(), formatDateShort()
...
Cohesion: 0.36
Nodes (8): AcademicCard(), WEEKDAYS, todayStr(), generateFreeWindows(), isoWeekday(), pad(), toStr(), putAvailability()
...
### Community 53 - "getDb"
Cohesion: 0.27
Nodes (9): POST(), DEFAULTS, GET(), PUT(), GET(), getAcceptedFriendIds(), getDb(), PreferencesSchema (+1 more)
...
Cohesion: 0.22
Nodes (7): Flight Scraper API — FastAPI application entry point.  Multi-user, no password, start_background_scheduler(), get_deals(), GET /api/deals — return flights matching the default user's preferences.  Uses, Service for matching flights to user preferences.      Usage:         matcher, UserMatcher, job_listener()
...
### Community 79 - "TargetsTable.tsx"
Cohesion: 0.29
Nodes (6): SortDir, SortKey, TargetsTable(), TargetsTableProps, TargetsTableSkeleton(), AdminTargetSummary

### Community 80 - "availability.py"
Cohesion: 0.29
...
Cohesion: 0.27
Nodes (8): nightsBetween(), TripRow(), TripRowProps, TripRowSkeleton(), Spark(), SparkPoint, SparkProps, getSearchUrl()
...
## Knowledge Gaps
- **246 isolated node(s):** `TripPopoverProps`, `TIER_BADGE`, `TripTooltipProps`, `MONTHS_SHORT`, `install.sh script` (+241 more)
...
- **Why does `UserRepository` connect `Community 7` to `Community 5`, `Community 38`, `Community 8`, `Community 46`, `Community 15`, `.create`, `.authenticate`?**
  _High betweenness centrality (0.061) - this node is a cross-community bridge._
- **Why does `DestinationRepository` connect `Community 7` to `Community 8`, `Community 38`, `Community 5`, `Community 14`?**
  _High betweenness centrality (0.029) - this node is a cross-community bridge._
- **Why does `FlightModel` connect `Community 31` to `Community 32`, `Community 7`, `Community 10`, `Community 43`, `Community 42`, `Community 12`, `Community 50`, `Community 54`, `Community 55`, `.find_by_date_range`?**
...
- **What connects `TripPopoverProps`, `TIER_BADGE`, `TripTooltipProps` to the rest of the system?**
