...
- **Settings → "Availabilities" IA (2026-06-13):** the Settings tab is renamed **Availabilities** (nav label in `components/layout/Navigation.tsx` + page H1; route stays `/settings`). Page section order is **Preferences → Quick setup → Free dates** (the old "Availability" section, renamed to avoid echoing the tab). `PreferencesCard` is stripped to **Departure airports** only (trip length, price ceiling, direct-only removed from the UI — still sent in the PUT payload as loaded so the `PreferencesSchema` contract is unchanged; "Save airports" button sits under the chips). Favourite-city selection now lives in Preferences via `components/settings/FavoriteCitiesPicker.tsx`: a typeahead over the **full `DESTINATIONS` catalogue** (works before any live deals exist), star-toggle rows + removable chips, backed by the same `useSavedCities()` store as the Explore ★ — so a favourite set here and the Explore stars are one source of truth, syncing both ways per user. Favourites persist instantly on toggle (no Save button); airports keep the explicit Save.
